...
public class BooksControllerTest {
    ...
    public void testShouldRedirectToPaymentIfBookInStock() throws Exception {
        ...
        BooksCollectionService booksCollectionService = mock(BooksCollectionService.class);
        BooksController controller = new BooksController(booksCollectionService, inventoryService);

        HttpServletRequest request = mock(HttpServletRequest.class);
        HttpSession session = mock(HttpSession.class);
        when(request.getSession(true)).thenReturn(session);
        Book desiredBook = new Book("isbn","name","auth", "pub");
        when(booksCollectionService.getBook("isbn")).thenReturn(desiredBook);

        ModelAndView modelAndView = controller.buyBook("isbn", request);
