...
public class BooksController {
    ...
    public ModelAndView buyBook(@RequestParam("isbn_check") String isbn_check, HttpServletRequest request) {
        HttpSession session = request.getSession(true);
        boolean exists = false;
        try {
            exists = booksInventoryService.checkInventory(isbn_check);
        } catch (BookNotFoundException e) {
        ...
        Book desiredBook = booksCollectionService.getBook(isbn_check);
        session.setAttribute("current_book", new BookCookie(desiredBook.getName(), isbn_check));
